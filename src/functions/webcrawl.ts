...
const webCrawl = async (data: { url: string }) => {
  try {
    console.info(`Entering webCrawl with url ${data.url}`);
    const response = await axios.get(data.url);
    
    const cleanedHtml = response.data.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
                                     .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '');
    
    var turndownService = new TurndownService();
    const markdown = turndownService.turndown(cleanedHtml);
    const truncatedMarkdown = markdown.slice(0, 10000);
    console.info(`Webcrawl finished the page with a total content length of ${truncatedMarkdown.length}`)
